...
def solve_qp(
    ...
    with gp.Env(empty=True) as env:
        ...
        with gp.Model(env=env) as m:
            w = m.addMVar(
                shape=cov_matrix.shape[0],
                lb=0,
                ub=max_concentration * SCALE,
                vtype=GRB.CONTINUOUS,
                name="w",
            )
            if return_vector is not None:
                m.setObjective(
                    w @ return_vector - risk_constant * w @ cov_matrix @ w, GRB.MAXIMIZE
                )
            else:
            ...
            try:
                ...
                return solve_qp(
                    cov_matrix + 5e-6 * np.eye(cov_matrix.shape[0]),
                    return_vector,
                    risk_constant,
...
class MarkowitzStrategy:
    ...
    def get_dollar_weights(self, backtester, adj_universe, price_by_ticker):
        ...
        weights = solve_qp(
            cov_matrix, return_vector, self._risk_constant, self._max_concentration
        )
        optimal_weights = {
            ticker: weight for ticker, weight in zip(adj_universe, weights)
        }
