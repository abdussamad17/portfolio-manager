...
class XGBStrategy:
        ...
        if self._regression:
            if self._negative:
                ...
                normalized_weights = {
                    ...
                    for ticker, weight in neg_weights.items()
                }
            else:
                ...
                normalized_weights = {
                    ticker: weight / total_weight
                    for ticker, weight in pos_weights.items()
                }
            ...
            normalized_weights = {
                ticker: weight / total_weight for ticker, weight in weights.items()
            }
    ...
    def infer(self, adj_universe, price_history):
        ...
        fix_id_by_adj_universe = {
            ...
            if not np.isnan(price_matrix[-input_window:, v]).any()
        }
        ...
        for i, (t, v) in enumerate(fix_id_by_adj_universe.items()):
            ...
            X_pred[i, :] = (
                price_series[1:input_window] / price_series[0 : input_window - 1] - 1.0
            )
    ...
    def train(self, price_history):
        ...
        for start_date_idx in range(0, idx_now - data_window):
            for stock_idx in range(0, price_matrix.shape[1]):
                ...
                price_series = price_matrix[
                    start_date_idx : start_date_idx + data_window, stock_idx
                ]
                X[i] = (
                    price_series[1 : input_window + 1] / price_series[0:input_window]
                    - 1.0
                )
