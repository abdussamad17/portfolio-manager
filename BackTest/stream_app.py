...
def load_json_data(file_path):
    ...
    if os.path.exists(local_path):
        ...
        st.info(
            "This strategy is not yet available. Please try again later or choose another strategy",
            icon="🚨",
        )
...
def setup_strategy(strategy_choice, i):
    ...
    if strategy_choice == "MarkowitzStrategy":
        ...
        risk_constant = st.sidebar.radio(
            f"Select Risk Constant for Strategy {i+1}", risk_constants
        )
        return_estimates = [0.000269, 0.00017, 0.00037]
        return_estimate = st.sidebar.radio(
            f"Select Return Estimate for Strategy {i+1}", return_estimates
        )
        vol_weighted = st.sidebar.checkbox(f"Volume Weighted for Strategy {i+1}?")
        max_concentrations = [0.05, 1]
        max_concentration = st.sidebar.radio(
            f"Select Max Concentration for Strategy {i+1}", max_concentrations
        )
        ...
        linkage_method = st.sidebar.selectbox(
            f"Select Linkage Method for Strategy {i+1}", linkage_methods
        )
        ...
        strategy_type_choice = st.sidebar.selectbox(
            f"Select Strategy Type for Strategy {i+1}", strategy_types
        )
...
def display_json_data(json_data_list, strategy_names):
    ...
    selected_date = st.date_input(
        "Choose a date to view portfolio constituents",
        value=pd.to_datetime(json_data_list[0][0]["date"]),
        min_value=pd.to_datetime(json_data_list[0][0]["date"]),
        max_value=pd.to_datetime(json_data_list[0][-1]["date"]),
    )
    ...
    for json_data, strategy_name in zip(json_data_list, strategy_names):
        selected_data = [
            ...
            if item["date"] == selected_date.strftime("%Y-%m-%d")
        ][0]
        ...
        constituents_fig.add_trace(
            go.Scatter(
                x=portfolio_df["Asset"],
                y=portfolio_df["Value"],
                mode="markers",
                marker=dict(size=portfolio_df["Weight"], sizemode="diameter"),
...
def plot_equity_curve(backtester, fig=None):
    dates = [
        ...
        for snap in backtester.snapshots
    ]
    ...
    fig.add_trace(
        go.Scatter(
            x=dates, y=portfolio_values, mode="lines", name=backtester.strategy_name
        )
    )
    fig.update_layout(
        legend=dict(
...
def plot_comparisons(json_data_list, strategy_names):
    ...
    for json_data, strategy_name in zip(json_data_list, strategy_names):
        ...
        df["date"] = pd.to_datetime(df["date"])
        rolling_metrics_fig.add_trace(
            go.Scatter(
                x=df["date"],
                y=df["roll_sigma"],
                mode="lines",
                name=f"{strategy_name} - Rolling Sigma",
            )
        )
        rolling_metrics_fig.add_trace(
            go.Scatter(
                ...
                y=df["roll_sr"],
                mode="lines",
                name=f"{strategy_name} - Rolling Sharpe Ratio",
            )
        )
        universe_size_fig.add_trace(
            go.Scatter(
                x=df["date"],
                y=df["n_stocks"],
                mode="lines",
                name=f"{strategy_name} - n_stocks",
            )
        )
        universe_size_fig.add_trace(
            go.Scatter(
                ...
                y=df["adj_uni_size"],
                mode="lines",
                name=f"{strategy_name} - Adjusted Universe Size",
                line=dict(dash="dash"),
            )
        )
        gini_fig.add_trace(
            go.Scatter(
                x=df["date"], y=df["gini"], mode="lines", name=f"{strategy_name}"
            )
        )

    rolling_metrics_fig.update_layout(
        title="Rolling Metrics Over Time: Comparison", xaxis_title="Date"
    )
    st.plotly_chart(rolling_metrics_fig)

    universe_size_fig.update_layout(
        title="Number of Stocks vs. Adjusted Universe Size: Comparison",
        xaxis_title="Date",
        yaxis_title="Size",
    )
    ...
    gini_fig.update_layout(
        title="Gini Coefficient Over Time: Comparison",
        xaxis_title="Date",
...
class EquityCurvesView:
    def view(self, json_data_list, strategies):
        ...
        for json_data, strategy_name in zip(json_data_list, strategies):
            ...
            fig.add_trace(
                go.Scatter(
                    x=dates, y=portfolio_values, mode="lines", name=strategy_name
                )
            )
        fig.update_layout(
            legend=dict(
...
class Model:
    ...
    def load_backtest_data(self):
        ...
        number_of_strategies = st.sidebar.slider(
            "Number of strategies to compare", 1, len(strategies)
        )
        json_data_list = []
        strategy_names = []
        ...
        for i in range(number_of_strategies):
            strategy_choice = st.sidebar.selectbox(
                f"Choose a Strategy {i+1}", strategies
            )
...
# Main function
st.set_page_config(page_title="Portfolio Backtest Visualizer", layout="wide")
view(Model())
