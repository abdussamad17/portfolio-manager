...
class HRPStrategy:
    ...
    def get_dollar_weights(self, backtester, adj_universe, price_by_ticker):
        ...
        ordered_dist_mat, res_order, res_linkage = self.compute_serial_matrix(dist_matrix, method=self._linkage_method)
        hrp_weights = self.compute_HRP_weights(cov_matrix, res_order)

        return {
            ticker_by_cov_id[idx]: w for idx, w in hrp_weights.items()
        }
    ...
    def seriation(Z, N, cur_index):
        ...
        if cur_index < N:
            ...
            return (HRPStrategy.seriation(Z, N, left) + HRPStrategy.seriation(Z, N, right))
